...
get_wc_2022_data <- function(file_name) {
  ...
  sports <- sports %>%
    dplyr::filter(str_detect(competitions.name, "2022 World Cup")) %>%
  ...
  wc_2022$scrape_date <- scrape_date

  wc_2022 <- dplyr::bind_rows(wc_2022, existing)

  # return(futures)
  save_to_release(df= wc_2022, file_name= file_name, release_tag= "sports")

}

# extract and save
get_wc_2022_data(file_name= "wc_2022_markets")
